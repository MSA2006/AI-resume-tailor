...
export async function generatePDF(data, mode = "professional") {
  ...
  const margin = mode === "internship" ? 45 : 55;
  const bodySize = mode === "internship" ? 9.5 : 10;
  const lineH = mode === "internship" ? 13 : 15;
  const headingSize = mode === "internship" ? 10 : 11;
  const nameSize = mode === "internship" ? 16 : 18;
  ...
  const drawEntryHeader = (title, organization, date) => {
    ...
    const leftMaxWidth = maxWidth - dateWidth - 10;

    // Draw title/org in bold on left
    let currentLine = "";
    const words = leftText.split(" ");
    let firstLine = true;

    // Simple single line for header — wrap if needed
    const fullLeft = leftText;
    const fullLeftWidth = boldFont.widthOfTextAtSize(fullLeft, bodySize);

    if (fullLeftWidth <= leftMaxWidth) {
      page.drawText(fullLeft, { x: margin, y, size: headingSize, font: boldFont, color: rgb(0, 0, 0) });
    } else {
      // Wrap left text without date on same line
      drawWrapped(fullLeft, boldFont, headingSize, rgb(0, 0, 0), lineH, 0);
    }

    // Draw date on the right of the same line
    if (dateText && fullLeftWidth <= leftMaxWidth) {
      const dateX = pageWidth - margin - dateWidth;
      page.drawText(dateText, { x: dateX, y, size: headingSize, font: boldFont, color: rgb(0.3, 0.3, 0.3) });
    }

    y -= 16;

    // If date couldn't fit on same line, draw it separately
    if (dateText && fullLeftWidth > leftMaxWidth) {
      drawWrapped(dateText, boldFont, headingSize, rgb(0.3, 0.3, 0.3), lineH, 0);
    }
  };
  ...
  for (const section of data.sections) {
    ...
    for (const entry of section.entries) {
      ...
      // Plain text — split by period or newline for skills
        ...
        if (skillLines.length > 1) {
          for (const skillLine of skillLines) {
            ...
            if (colonIndex !== -1) {
              ...
              page.drawText(" " + rest, { x: margin + labelWidth, y, size: bodySize, font: regularFont, color: rgb(0.1, 0.1, 0.1) });
              y -= lineH;
            } else {
